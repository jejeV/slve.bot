...
client.on("messageCreate", (message) => {
    ...
    if (command === 'hi') {
        message.channel.send("hi pipi")
    }

    //ban command

    if (command === 'ban'){
         const member = message.mentions.members.first() || message.guild.members.cache.get(argument[0]) || message.guild.members.cache.find(x => x.user.username.toLowerCase() === argument.slice(0).join(" " || x.username === argument[0]));
         ...
         member.ban().catch(err => {
            message.channel.send("There wan an error banning this member.");
         })
...
client.login("MTA1MjQzODA0Njk4Mjc5OTQxMQ.Gm9bU6.ZsqvypgMDaSM7bBQWuXv5DmQcwZTvxwIpQgDDo");